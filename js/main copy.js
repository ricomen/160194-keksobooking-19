...
var fillAddFormAddress = function () {
  var getMainPinCoords = function () {
    var mapCoords = getCoords(map);
    var pinWidth = MainPinSize.WIDTH;
    var pinHeight = MainPinSize.HEIGHT;
    var pinCoordsX = Math.round(getCoords(mapPinMain).left + pinWidth / 2 - mapCoords.left);
    var pinCoordsY = Math.round(getCoords(mapPinMain).top + pinHeight - mapCoords.top);
...
var mapPinMainHandler = function () {
  if (pageIsActive) {
    ...
    activatePage(true);
    window.dnd.init(fillAddFormAddress);
